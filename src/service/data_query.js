...
class DataQuery {
    ...
    read_by_id(id = 0) {
        return new Promise((resolvem, reject) => {
            this.connection.execute('select count(email) from user where id = ?', [id], (err, res) => {
                if (err) reject(err)
                resolvem(res)
            })
        })
    ...
    read_by_name(user_name = '') {
        return new Promise((resolve, reject) => {
            this.connection.execute('select count(email) from user where nombre = ?', [user_name], function (err, res) {
                if (err) reject(err)
                resolve(res)
            })
        })
    ...
    read_by_email(user_email = '') {
        return new Promise((resolve, reject) => {
            this.connection.execute('select count(email) from user where email = ?', [user_email], function (err, res) {
                if (err) reject(err)
                resolve(res)
            })
        })
